...
public class OrgNm_Party_Test extends driverSelection{
	public static Logger log=LogManager.getLogger(OrgNm_Party_Test.class.getName());	
	@Test
	public void Party_Val() throws IOException, InterruptedException
	{
	...
	org.lgn().click();
	d.until(ExpectedConditions.visibilityOfAllElements(org.Iddselection_Org_Party()));
	Assert.assertEquals(org.Iddselection_Org_Party().getText(), "FA PARTY IDD App");
	log.info("successfully logged in");
	System.out.println("loginpass");
	String idds1=org.Iddselection_Org_Party().getText();
	System.out.println(idds1);
	
	////IDD selection
	
	org.Iddselection_Org_Party().click();
	Thread.sleep(10000);
	Assert.assertEquals(org.iddSelectionTest().getText(), "Queries");
	log.info("successfully selected "+idds1+"- Org");
